...
VL_INLINE_OPT void Vlng___024root___nba_sequent__TOP__0(Vlng___024root* vlSelf) {
    ...
    // Init
    SData/*15:0*/ __Vdly__temp2dig__DOT__count;
    __Vdly__temp2dig__DOT__count = 0;
    // Body
    __Vdly__temp2dig__DOT__count = vlSelf->temp2dig__DOT__count;
    if (vlSelf->reset) {
        vlSelf->rst = 1U;
        vlSelf->d_out = 0U;
        __Vdly__temp2dig__DOT__count = 0U;
    } else {
        vlSelf->rst = 0U;
        if (vlSelf->vop) {
            __Vdly__temp2dig__DOT__count = (0xffffU 
                                            & ((IData)(1U) 
                                               + (IData)(vlSelf->temp2dig__DOT__count)));
        } else {
            vlSelf->d_out = vlSelf->temp2dig__DOT__count;
        }
    }
    vlSelf->temp2dig__DOT__count = __Vdly__temp2dig__DOT__count;
}
...
#endif
            VL_FATAL_MT("temp_to_digital.v", 23, "", "NBA region did not converge.");
        }
...
#endif
                VL_FATAL_MT("temp_to_digital.v", 23, "", "Active region did not converge.");
            }
...
void Vlng___024root___eval_debug_assertions(Vlng___024root* vlSelf) {
    ...
    if (VL_UNLIKELY((vlSelf->vop & 0xfeU))) {
        Verilated::overWidthError("vop");}
}
#endif  // VL_DEBUG
